import { FOOT_Y } from './constants.js';
...
// dans la forme de verre choisie sans toucher les parois — jusqu'à `fill` du
// rayon du verre (la moitié par défaut ; une boule duveteuse supporte plus
// d'approcher les parois qu'une fleur aux pétales nets). `ground` dit si le
...
export const OBJECTS = {
  ...
  thistle: { label: 'Chardon', create: createThistle, footY: THISTLE_FOOT_Y, wideY: 0.02, reach: 0.5, topY: THISTLE_TOP_Y, ground: false, floating: true, fill: 0.72 },
};

// Facteur d'échelle pour que l'objet, pied posé à `footTarget`, reste à
// distance des parois : sa largeur ne dépasse pas `fill` du rayon du verre à
...
export function fitScale(def, shape, footTarget) {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    const roomWide = ((def.fill ?? 0.5) * shape.radiusAt(Math.min(wideAt, shape.topY - 0.01))) / def.reach;
